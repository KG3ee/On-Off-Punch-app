...
export class PayrollController {
  ...
  @Post('salary-rules')
  async createSalaryRule(@Body() dto: CreateSalaryRuleDto, @CurrentUser() actor: AuthUser) {
  ...
  @Post('runs/generate')
  async generateRun(@Body() dto: GeneratePayrollRunDto, @CurrentUser() actor: AuthUser) {
    return this.payrollService.generateRun(dto, actor);
  }

  @Post('runs/:id/finalize')
  async finalizeRun(@Param('id') runId: string, @CurrentUser() actor: AuthUser) {
